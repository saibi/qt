...
void GroceryItems::sortBy(const QString &field, const QString &direction)
{
	if (field == "name" && direction == "ASC") {
		std::sort( m_list.begin(), m_list.end(), [](const QVariant &v1, const QVariant &v2) { return v1.toMap().value("name") < v2.toMap().value("name"); });
...
void GroceryItems::setRepository(repositories::GroceryItemsRepo *repository)
{
	...
	connect(m_repository, &repositories::GroceryItemsRepo::allRecordsRetrieved, this, &GroceryItems::onAllRecordsRetrieved, Qt::UniqueConnection);
}
