...
#define BEEP_OFF_STR " "
#endif
#define BEEP_STR_LEN 1	// strlen(BEEP_ON_STR), strlen(BEEP_OFF_STR)
...
#ifdef __arm__
			if (_fdBeep)
				if ( ::write(_fdBeep, BEEP_ON_STR, BEEP_STR_LEN) != BEEP_STR_LEN )
					continue;
#endif
...
#ifdef __arm__
			if (_fdBeep)
				if ( ::write(_fdBeep, BEEP_OFF_STR, BEEP_STR_LEN) != BEEP_STR_LEN )
					continue;
#endif
