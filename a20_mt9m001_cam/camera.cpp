...
int vid_s_ctrl(int id, int value)
{
	...
	return ioctl(fd, VIDIOC_S_CTRL, &control_s);
}
...
int vid_q_ctrl(int id, struct v4l2_queryctrl *p)
{
	...
	if ( p )
	{
		...
		return ioctl(fd, VIDIOC_QUERYCTRL, p);
	}
...
static int read_frame(void)
{
	...
	case IO_METHOD_USERPTR:
		...
		for (i = 0; i < MAX_CAM_BUFFER; ++i)
			if (buff.m.userptr == (unsigned long) buffers[i].start && buff.length == buffers[i].length)
				break;
...
static void init_mmap(void)
{
	...
	for (i_buffers = 0; i_buffers < (int) req.count; i_buffers++)
	{
		...
		buffers[i_buffers].start = mmap(NULL /* start anywhere */ ,
						v4l_buf[i_buffers].length, PROT_READ | PROT_WRITE /* required */ ,
